...
import { type OrderRequest, OrderStatus, OrderType } from '@/types/models'
...
export default function DevolutionOrdersPage() {
  ...
  const { orders, loading, error, fetchResellerOrders } = useOrdersStore()
  ...
  const load = useCallback(() => {
    const params: { page: number; limit: number; status?: string; orderType: string } = {
      page: page + 1,
      limit,
  ...
  }, [page, limit, statusFilter, fetchResellerOrders])
  ...
  const columns = useMemo(() => ([
    ...
    {
      key: 'devolutionReason',
      label: 'Reason',
      render: (o: OrderRequest) => o.devolutionReason ?? '—',
    },
    {
      key: 'createdAt',
      label: 'Created',
      render: (o: OrderRequest) => o.createdAt ? new Date(o.createdAt).toLocaleDateString() : '—',
    },
    {
      key: 'actions',
      label: 'Actions',
      ...
      render: (o: OrderRequest) => (
        <Button
          ...
          onClick={(e) => { e.stopPropagation(); navigate(ROUTES.ORDER_DETAIL.replace(':id', o._id)) }}
        >
          View
        </Button>
      ),
  ...
  return (
    <Box>
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">Devolutions</Typography>
      </Breadcrumbs>
      ...
      <DataTable<OrderRequest>
        ...
        emptyMessage="No devolution orders found."
      />
    </Box>
  )
}
